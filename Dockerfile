...
ENV OULU_URL https://github.com/charliewolf/oulu/archive/threading.zip
RUN curl -L $OULU_URL -o /tmp/oulu.zip && \
    unzip /tmp/oulu.zip -d /opt/ && \
    mv /opt/oulu* /opt/oulu/ && \
    (cd /opt/oulu && bundle install) && \
